                ...
                <div class="card">
                    ...
                    <div class="card-body bg-secondary">
                        <form wire:submit.prevent="saveOrigin" autocomplete="off">                       
                            <div class="px-lg-2">
                                <div class="row">
                                    <div wire:ignore class="form-group{{ $errors->has('packing_list') ? ' has-danger' : '' }} col-md-6 px-2 mb-2">
                                        <label class="form-control-label" for="input-packing_list">{{ __('Packing List') }}</label>
                                        <select
                                            name="packing_list[]"
                                            id="input-packing_list"
                                            class="form-control form-control-alternative{{ $errors->has('packing_list') ? ' is-invalid' : '' }}"
                                            required
                                            autofocus
                                            wire:model="packing_list"
                                            multiple>
                                            @foreach ($packings as $packing)
                                                <option value="{{$packing->packing_list_number}}">{{$packing->packing_list_number}}</option>
                                            @endforeach
                                        </select>
...
@push('js')
    <script>
        $(document).ready(function() {
            $('#input-packing_list').select2();
            $('#input-packing_list').on('change', function (e) {
                var data = $('#input-packing_list').select2("val");
                @this.set('packing_list', data);
            });
        });
    </script>
@endpush
